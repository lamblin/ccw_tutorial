...
\begin{frame}{Objectives}
  This session will have 5 parts
  \begin{itemize}
    \item Introduction to Theano
    \item Hands-on exercises on the basics of Theano
    \item Debugging in Theano
    \item Scan: symbolic loops in Theano
    \item Hands-on exercises on scan
  \end{itemize}

  \url{http://github.com/lamblinp/ccw_tutorial/}
\end{frame}
...
\begin{frame}[fragile]{Features recently added to Theano}
  \begin{itemize}
    \item Integration of CuDNN v3 for 2D/3D convolutions and pooling
    ...
    \item New GPU backend (beta), with arrays of all dtypes, for CUDA and OpenCL
      \begin{itemize}
        \item Support for half-precision float (\verb|float16|) for some operations
      \end{itemize}
...
\begin{frame}[fragile]{Thanks for your attention}
  ...
  \uncover<2->{
    ...
    \begin{itemize}
      \item Slides: {\tt Theano\_A2015/intro\_theano.pdf}
      \item Notebook with the code examples: {\tt Theano\_A2015/intro\_theano.ipynb}
    \end{itemize}
  }
\end{frame}
